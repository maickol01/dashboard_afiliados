...
export const useData = () => {
  ...
  const validateLeaderPerformanceData = useCallback((data: unknown[]): unknown[] => {
    if (!Array.isArray(data)) {
    ...
    return data.filter(item => {
      ...
      if (typeof item.name !== 'string' || !item.name.trim()) {
        console.warn('Invalid leader name: expected non-empty string, got:', typeof item.name);
        return false;
      }
      
      if (typeof item.citizenCount !== 'number' || isNaN(item.citizenCount) || item.citizenCount < 0) {
        console.warn('Invalid citizen count: expected non-negative number, got:', item.citizenCount);
        return false;
      }
